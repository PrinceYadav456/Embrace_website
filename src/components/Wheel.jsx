...
const Wheel = ({ projects }) => {
  ...
  const getResponsiveRadius = () => {
    if (width < 480) return 500;      // small phone
    if (width < 768) return 350;      // large phone / small tablet
  ...
  const getResponsiveLeftOffset = () => {
    ...
    return "10px";                     // desktop – you can adjust
  };
  ...
  const rawAngle = useTransform(scrollYProgress, [0, 1], [0, 2 * Math.PI]);
  ...
  return (
    <>
      <div ref={scrollRef} className="wheel-scroll-container" />
      <div
        className="wheel-fixed"
        style={{ left: leftOffset }}   // ← dynamic left offset
      >
